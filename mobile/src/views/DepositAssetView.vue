...
const filteredAssets = computed(() => {
  ...
  return keyword
    ? assets.value.filter((item) => item.symbol.includes(keyword) || item.name?.toUpperCase().includes(keyword))
    : assets.value
})
...
<template>
  <main
    class="page page--plain pencil-page wallet-pencil-page deposit-asset-pencil"
    data-pencil-source="fNXT7 n5jiPN"
  >
    <PageHeader
      :back="true"
    ...
    <div class="page-content asset-page">
      <LoginRequiredState
        ...
        :description="t('deposit.assetLoginDescription')"
      />
      <template v-else>
        ...
        <div v-else-if="filteredAssets.length" class="asset-picker">
          <button v-for="asset in filteredAssets" :key="asset.symbol" type="button" @click="selectAsset(asset)">
            ...
            <span>
              <b>{{ asset.symbol }}</b>
              <small>{{ asset.name || t('deposit.supported') }}</small>
            </span>
