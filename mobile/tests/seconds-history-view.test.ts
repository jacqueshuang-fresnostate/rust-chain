...
test('历史页完整展示 API 字段且缺失结算价不使用实时价替代', () => {
  ...
  assert.match(historyTemplate, /t\('seconds\.historyEntryPrice'\)[\s\S]*?order\.entryPriceText \? formatPrice\(order\.entryPriceText\) : '--'/)
  assert.match(historyTemplate, /t\('seconds\.historySettlementPrice'\)[\s\S]*?order\.settlementPriceText \? formatPrice\(order\.settlementPriceText\) : '--'/)
  ...
  assert.match(historySource, /function orderProfitLossAmount\(order: SecondsOrder\): string \{[\s\S]*?secondsOrderProfitLossPresentation\(order\)[\s\S]*?!presentation\.amountText[\s\S]*?decimalSign\(presentation\.amountText\) > 0 \? '\+' : ''[\s\S]*?order\.stakeAssetSymbol/)
