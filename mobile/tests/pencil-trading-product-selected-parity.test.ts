...
test('杠杆为独立二栏下单、六卖七买真实盘口和真实持仓风险面', () => {
  ...
  assert.match(orderBookSource, /\.order-book__mini-ratio\s*\{[\s\S]*?grid-template-columns: minmax\(0, 1fr\) minmax\(0, 1fr\);/)
  assert.match(orderBookSource, /\.order-book__mini-ratio-track\s*\{[\s\S]*?grid-column: 1 \/ -1;[\s\S]*?position: relative;/)
  assert.doesNotMatch(tradeSource, /\.order-book__mini-ratio\)::before/)
  ...
  assert.doesNotMatch(tradeSource, /marginCrossAccounts|MarginCrossAccount/)
  ...
  assert.match(tradeSource, /resolveMarginPositionRiskMetrics\(\{[\s\S]*?serverMaintenanceMarginRate:[\s\S]*?productMaintenanceMarginRate:[\s\S]*?serverEstimatedLiquidationPrice:[\s\S]*?serverLiquidationDistanceRate:[\s\S]*?crossAccountRisk:/)
...
test('产品中心仅渲染两条 64px 产品行与一条 48px 产品说明入口', () => {
  ...
  assert.match(productHubSource, /class="page page--plain pencil-page product-hub"/)
  ...
  assert.match(productHubSource, /<Ellipsis :size="18"/)
  ...
  assert.match(productHubSource, /class="icon-button"[^>]*:aria-label="t\('products\.hubHelp'\)"[^>]*@click="openProductHelp"/)
  assert.doesNotMatch(productHubSource, /d2FQe|CircleDollarSign|products\.introDescription|news\.market/)
  assert.deepEqual({
...
test('产品中心正常态几何、字体与无分割线合同精确匹配 Z0B0N6/zMsKE', () => {
  const css = styleOf(productHubSource)
  const rootRule = blockOf(css, '.product-hub {')
  const bodyRule = blockOf(css, '.product-hub__body {')
  const listRule = blockOf(css, '.product-hub__list {')
  const rowRule = blockOf(css, '.product-hub__row {')
  const iconRule = blockOf(css, '.product-hub__row .product-card__icon {')
  const copyRule = blockOf(css, '.product-hub__copy {')
  const titleRule = blockOf(css, '.product-hub__copy strong {')
  const descriptionRule = blockOf(css, '.product-hub__copy small {')
  const rowChevronRule = blockOf(css, '.product-hub__row > svg {')
  const helpRule = blockOf(css, '.product-hub__help {')
  const helpIconRule = blockOf(css, '.product-hub__help > svg:first-child {')
  const helpChevronRule = blockOf(css, '.product-hub__help > svg:last-child {')
  const helpLabelRule = blockOf(css, '.product-hub__help span {')

  assert.equal(cssValue(rootRule, 'font-family'), 'var(--font-geist-sans), "PingFang SC", "Microsoft YaHei", sans-serif')
  assert.equal(cssValue(rootRule, 'display'), 'block')
  assert.equal(cssValue(rootRule, 'gap'), '0')
  assert.equal(cssValue(bodyRule, 'gap'), '18px')
  assert.equal(cssValue(bodyRule, 'grid-template-rows'), '128px 48px')
  assert.equal(cssValue(bodyRule, 'min-width'), '0')
  assert.equal(cssValue(bodyRule, 'padding'), '8px 20px calc(20px + env(safe-area-inset-bottom))')
  assert.equal(cssValue(listRule, 'grid-template-rows'), 'repeat(2, 64px)')
  assert.equal(cssValue(listRule, 'height'), '128px')
  assert.equal(cssValue(listRule, 'min-width'), '0')

  assert.equal(cssValue(rowRule, 'background'), 'transparent')
  assert.equal(cssValue(rowRule, 'border'), '0')
  assert.equal(cssValue(rowRule, 'border-radius'), '0')
  assert.equal(cssValue(rowRule, 'box-shadow'), 'none')
  assert.equal(cssValue(rowRule, 'gap'), '14px')
  assert.equal(cssValue(rowRule, 'height'), '64px')
  assert.equal(cssValue(rowRule, 'min-height'), '64px')
  assert.equal(cssValue(rowRule, 'padding'), '0')
  assert.equal(cssValue(rowRule, 'width'), '100%')
  assert.doesNotMatch(rowRule, /border-bottom/)

  assert.equal(cssValue(iconRule, 'background'), 'var(--surface-elevated)')
  assert.equal(cssValue(iconRule, 'border'), '1px solid var(--line)')
  assert.equal(cssValue(iconRule, 'border-radius'), '22px')
  assert.equal(cssValue(iconRule, 'box-shadow'), 'none')
  assert.equal(cssValue(iconRule, 'color'), 'var(--text)')
  assert.equal(cssValue(iconRule, 'display'), 'grid')
  assert.equal(cssValue(iconRule, 'flex'), '0 0 44px')
  assert.equal(cssValue(iconRule, 'height'), '44px')
  assert.equal(cssValue(iconRule, 'place-items'), 'center')
  assert.equal(cssValue(iconRule, 'width'), '44px')
  assert.doesNotMatch(css, /data-product=["']news["'][^{]*\.product-card__icon/)

  assert.equal(cssValue(copyRule, 'gap'), '3px')
  assert.equal(cssValue(copyRule, 'height'), '41px')
  assert.equal(cssValue(copyRule, 'min-width'), '0')
  assert.equal(cssValue(titleRule, 'font-size'), '15px')
  assert.equal(cssValue(titleRule, 'font-weight'), '700')
  assert.equal(cssValue(titleRule, 'line-height'), '22px')
  assert.equal(cssValue(titleRule, 'text-overflow'), 'ellipsis')
  assert.equal(cssValue(titleRule, 'white-space'), 'nowrap')
  assert.equal(cssValue(descriptionRule, 'color'), 'var(--product-hub-muted)')
  assert.equal(cssValue(descriptionRule, 'font-size'), '11px')
  assert.equal(cssValue(descriptionRule, 'font-weight'), '450')
  assert.equal(cssValue(descriptionRule, 'line-height'), '16px')
  assert.equal(cssValue(descriptionRule, 'text-overflow'), 'ellipsis')
  assert.equal(cssValue(descriptionRule, 'white-space'), 'nowrap')
  assert.equal(cssValue(rowChevronRule, 'color'), 'var(--product-hub-muted)')

  assert.equal(cssValue(helpRule, 'background'), 'transparent')
  assert.equal(cssValue(helpRule, 'border'), '0')
  assert.equal(cssValue(helpRule, 'border-radius'), '0')
  assert.equal(cssValue(helpRule, 'box-shadow'), 'none')
  assert.equal(cssValue(helpRule, 'color'), 'var(--text)')
  assert.equal(cssValue(helpRule, 'font-size'), '13px')
  assert.equal(cssValue(helpRule, 'font-weight'), '650')
  assert.equal(cssValue(helpRule, 'height'), '48px')
  assert.equal(cssValue(helpRule, 'line-height'), '19px')
  assert.equal(cssValue(helpRule, 'min-height'), '48px')
  assert.doesNotMatch(helpRule, /border-bottom/)
  assert.equal(cssValue(helpIconRule, 'color'), 'var(--text)')
  assert.equal(cssValue(helpChevronRule, 'color'), 'var(--product-hub-muted)')
  assert.equal(cssValue(helpChevronRule, 'margin-left'), 'auto')
  assert.equal(cssValue(helpLabelRule, 'flex'), '0 1 auto')
  assert.equal(cssValue(helpLabelRule, 'min-width'), '0')
  assert.equal(cssValue(helpLabelRule, 'text-overflow'), 'ellipsis')
  assert.equal(cssValue(helpLabelRule, 'white-space'), 'nowrap')
  assert.doesNotMatch(css, /padding-inline:\s*16px|border-bottom:|\bwidth:\s*350px|\banimation(?:-name)?:/)

  const compiled = compileStyle({ source: css, filename: 'ProductHubView.vue', id: 'data-v-product-hub', scoped: true })
  assert.deepEqual(compiled.errors, [])
  assert.match(compiled.code, /\.product-hub\[data-v-product-hub\]\s*\{[\s\S]*?display: block;[\s\S]*?gap: 0;/)
})

test('产品中心明暗色复用 Pencil 语义变量并保持 Header 尺寸', () => {
  const css = styleOf(productHubSource)
  const productTokens = blockOf(selectedPageCss, '.app-stage .mobile-canvas .product-hub{')
  const lightTokens = blockOf(selectedPageCss, '.pencil-page,')
  const darkTokens = blockOf(selectedPageCss, "html[data-theme='dark'] .pencil-page,")
  const headerCss = styleOf(pageHeaderSource)
  const headerRule = blockOf(headerCss, '.pencil-page-header {')
  const headerTitleRule = blockOf(headerCss, '.pencil-page-header .page-header__title {')
  const headerActionRule = blockOf(headerCss, '.pencil-page-header :deep(.icon-button) {')

  assert.equal(cssValue(productTokens, '--product-hub-muted'), '#7a8b80')
  assert.equal(cssValue(lightTokens, '--text'), '#111714')
  assert.equal(cssValue(lightTokens, '--surface-elevated'), '#ffffff')
  assert.equal(cssValue(lightTokens, '--line'), '#ccd5d0')
  assert.equal(cssValue(darkTokens, '--text'), '#f2f7f4')
  assert.equal(cssValue(darkTokens, '--surface-elevated'), '#0c100e')
  assert.equal(cssValue(darkTokens, '--line'), '#29342e')
  assert.doesNotMatch(selectedPageCss, /(?:^|\n)\.product-hub\s*\{/)
  assert.doesNotMatch(`${selectedPageCss}\n${css}`, /--ph-[a-z]|--product-hub-(?:text|plate)/)

  assert.equal(cssValue(headerRule, 'grid-template-columns'), '44px minmax(0, 1fr) 44px')
  assert.equal(cssValue(headerRule, 'height'), '60px')
  assert.equal(cssValue(headerRule, 'min-height'), '60px')
  assert.equal(cssValue(headerRule, 'padding'), '8px var(--pencil-header-inline, 18px)')
  assert.equal(cssValue(headerTitleRule, 'font-size'), '18px')
  assert.equal(cssValue(headerTitleRule, 'font-weight'), '750')
  assert.equal(cssValue(headerTitleRule, 'line-height'), '26px')
  assert.equal(cssValue(headerActionRule, 'height'), '44px !important')
  assert.equal(cssValue(headerActionRule, 'width'), '44px !important')
  assert.match(pageHeaderSource, /<ArrowLeft :size="pencil \? 22 : 20"/)
  assert.match(productHubSource, /<Ellipsis :size="18"/)
})
...
test('预测页使用 pU7Kz/IcvzQ 的真实市场卡、状态筛选与是/否报价动作', () => {
  ...
  assert.match(predictionSource, /const stakeAmount = amountText\.value[\s\S]*requestPredictionQuote\(\{[\s\S]*marketId: selected\.value\.id,[\s\S]*outcome: outcome\.value,[\s\S]*assetId: assetId\.value,[\s\S]*stakeAmount,[\s\S]*\}\)/)
