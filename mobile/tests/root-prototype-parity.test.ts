...
test('根视图继续调用真实 API/store，访客与加载错误状态不改变原型骨架', () => {
  ...
  assert.match(views.trade, /await placeMarginOrder\(\{\s*\.\.\.review\.request,\s*marginAmount: review\.marginAmountText,\s*price: review\.request\.price,\s*\}\)/)
  assert.match(views.assets, /fetchWalletAccounts\(\)/)
  assert.match(views.assets, /fetchMarginWallets\(\)/)
  assert.match(views.assets, /const requestAmount = transferValueText\.value[\s\S]*await transferWalletFunds\(transferAsset\.value,\s*transferFrom\.value,\s*to,\s*requestAmount\)/)
