...
test('交易页保留现货、合约和 mode 路由合同', () => {
  ...
  assert.match(tradeSource, /navigation\.rememberTradeMode\(nextMode\)/)
  assert.match(tradeSource, /void router\.replace\(\{[\s\S]*query: nextMode === 'contract' \? \{ mode: 'contract' \} : undefined,/)
