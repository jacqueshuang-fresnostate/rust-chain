...
test('交易页保留现货、合约和 mode 路由合同', () => {
  assert.match(tradeSource, /const spot = spotReview\.value[\s\S]*await placeSpotOrder\(\{\s*symbol: spot\.symbol,\s*side: spot\.side,\s*type: spot\.orderType,\s*price: spot\.price,\s*quantity: spot\.quantity,/)
  assert.match(tradeSource, /createMarginOrderReview\(\{\s*productId: selectedProduct\.value\?\.id \|\| 0,\s*side: side\.value,\s*marginMode: marginMode\.value,\s*leverage: leverage\.value,\s*marginAmount: quantity\.value,/)
  ...
  assert.match(tradeSource, /percentagePoints: normalizedPercent/)
...
test('秒合约页保留真实产品、钱包、下单和活动订单合同', () => {
  ...
  assert.match(
    secondsSource,
    /function clearSecondsPrivateState\(\): void \{\s*orders\.value = \[\]\s*accounts\.value = \[\][\s\S]*?resultTracker\.reset\(\)[\s\S]*?clearSettlementResultQueue\(\)\s*\}/,
  )
  assert.match(secondsSource, /const review = orderReview\.value[\s\S]*?await openSecondsOrder\(\{\s*productId: review\.productId,\s*durationSeconds: review\.durationSeconds,\s*direction: review\.direction,\s*stakeAmount: review\.stakeAmountText,\s*idempotencyKey: review\.idempotencyKey,/)
  ...
  assert.match(secondsSource, /v-if="!session\.isAuthenticated"[\s\S]*?v-else-if="loading"[\s\S]*?moneyText\(availableStakeBalance\)/)
...
test('借贷页保留真实申请、撤销、还款并开放逾期还款', () => {
  assert.match(loanSource, /const requestAmount = amountText\.value[\s\S]*const requestCollateral = selected\.value\.loanType === 'collateralized'[\s\S]*await applyLoan\(\{[\s\S]*productId: selected\.value\.id,[\s\S]*amount: requestAmount,[\s\S]*collateralAssetId:[\s\S]*collateralAmount:[\s\S]*requestCollateral/)
...
test('借贷抵押资产使用带后端 Logo 的可访问底部弹窗', () => {
  ...
  assert.match(walletSource, /logoUrl: 'logo_url' in account \? account\.logo_url\?\.trim\(\) \|\| undefined : undefined/)
