...
test('秒合约保持独立真实产品工作台和市场参考价', () => {
  ...
  assert.match(secondsSource, /class="seconds-cycle-limit"/)
  assert.match(secondsSource, /class="seconds-orders-workspace"/)
...
test('图表和盘口在空数据与重复时间点下仍保留稳定画布', () => {
  ...
  assert.match(chartSource, /createMarketChartHistorySession\(\{/)
  assert.match(chartSource, /const normalizedPoints = computed\(\(\) => history\.value\.points\)/)
