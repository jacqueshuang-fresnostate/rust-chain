...
test('首页 390px 块级几何与三行真实行情按选中画板固定', () => {
  ...
  assert.match(parityCss, /\.home-view \.market-brief\s*\{[\s\S]*?border-radius:\s*20px;[\s\S]*?margin:\s*10px 16px 8px;[\s\S]*?min-height:\s*166px;/)
  assert.match(parityCss, /\.theme-light \.mobile-canvas \.home-view \.market-brief\s*\{[\s\S]*?linear-gradient\(145deg,[\s\S]*?#edf8f3 76%\);[\s\S]*?color:\s*#11231b;/)
  assert.match(parityCss, /\.market-brief__hero\s*\{[\s\S]*?justify-content:\s*space-between;/)
  assert.match(parityCss, /\.market-brief__quotes\s*\{[\s\S]*?grid-template-columns:\s*minmax\(0, 1\.28fr\) minmax\(0, 0\.92fr\);/)
  assert.doesNotMatch(parityCss, /\.home-view \.market-brief:disabled/)
...
test('根 Dock 只保留五入口与 56px 中央交易，独立产品路由仍可达', () => {
  ...
  assert.match(navSource, /navigation\.rememberTradeMode\('spot'\)/)
  assert.doesNotMatch(navSource, /selectedTradeDestination|option\.active/)
  assert.match(navSource, /data-pencil-source="X0ux9F RtubA U99rP n1BXc eLvdo QrlAB"/)
