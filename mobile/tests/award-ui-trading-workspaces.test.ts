...
test('交易工作台保留行情、K 线、盘口、余额、委托与下单处理函数', () => {
  ...
  assert.match(tradeSource, /const spot = spotReview\.value[\s\S]*?await placeSpotOrder\(\{[\s\S]*?symbol: spot\.symbol,[\s\S]*?quantity: spot\.quantity,/)
  assert.match(tradeSource, /createMarginOrderReview\(\{[\s\S]*?productId: selectedProduct\.value\?\.id \|\| 0,[\s\S]*?marginAmount: quantity\.value,/)
...
test('秒合约继续直用现货钱包、后台产品周期与真实下单接口且没有划转入口', () => {
  ...
  assert.match(secondsSource, /const review = orderReview\.value[\s\S]*?await openSecondsOrder\(\{[\s\S]*?productId: review\.productId,[\s\S]*?durationSeconds: review\.durationSeconds,[\s\S]*?direction: review\.direction,[\s\S]*?stakeAmount: review\.stakeAmountText,[\s\S]*?idempotencyKey: review\.idempotencyKey,/)
