...
test('订单页保留现货、杠杆查询与逐笔/批量撤单平仓合同', () => {
  ...
  assert.match(ordersSource, /fetchOpenSpotOrders\(30, signal\)/)
  assert.match(ordersSource, /fetchSpotOrderHistory\(30, signal\)/)
  assert.match(ordersSource, /fetchMarginPositions\('opened', 30, signal\)/)
  assert.match(ordersSource, /fetchMarginPositions\('closed', 30, signal\)/)
  assert.match(ordersSource, /fetchMarginPositions\('liquidated', 30, signal\)/)
  assert.match(ordersSource, /fetchMarginPositions\('canceled', 30, signal\)/)
