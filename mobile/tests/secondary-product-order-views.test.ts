...
const transactionRecordsSource = readFileSync(new URL('../src/composables/useTransactionRecords.ts', import.meta.url), 'utf8')
const transactionLayoutSource = readFileSync(new URL('../src/components/TransactionRecordsLayout.vue', import.meta.url), 'utf8')
const sources = [
  ordersSource,
  swapSource,
  earnSource,
...
test('订单页保留现货、杠杆查询与逐笔/批量撤单平仓合同', () => {
  ...
  assert.match(ordersSource, /<TransactionRecordsLayout/)
  assert.match(transactionLayoutSource, /v-for="tab in visibleTabs"/)
  assert.match(transactionRecordsSource, /fetchOpenSpotOrders\(30, signal\)/)
  assert.match(transactionRecordsSource, /fetchSpotOrderHistory\(30, signal\)/)
  assert.match(transactionRecordsSource, /fetchMarginPositions\('opened', 30, signal\)/)
  assert.match(transactionRecordsSource, /fetchMarginPositions\('closed', 30, signal\)/)
  assert.match(transactionRecordsSource, /fetchMarginPositions\('liquidated', 30, signal\)/)
  assert.match(transactionRecordsSource, /fetchMarginPositions\('canceled', 30, signal\)/)
  assert.match(transactionRecordsSource, /fetchMarketPairs\(\)/)
  assert.match(ordersSource, /product\.id === position\.productId \|\| product\.pairId === position\.pairId/)
  assert.match(ordersSource, /pair\.id === position\.pairId/)
  assert.doesNotMatch(tradingApiSource, /symbol:\s*String\(position\.[^)]*pair_id/)
  assert.match(ordersSource, /await cancelSpotOrder\(action\.id\)/)
  assert.match(ordersSource, /cancelAllSpotOrders\(\)\)\.failures\.length/)
  assert.match(ordersSource, /await cancelMarginPosition\(action\.id\)/)
  ...
  assert.match(ordersSource, /watch\(\(\) => route\.query\.tab/)
  assert.match(ordersSource, /normalizeTransactionRecordTab\(tab\)/)
})
...
test('闪兑与理财页保留实时报价、确认、申购和赎回链路', () => {
  ...
  assert.match(swapSource, /const requestAmount = amountText\.value[\s\S]*quote\.value = await requestConvertQuote\(selectedPair\.value, requestAmount\)/)
  ...
  assert.match(earnSource, /const requestAmount = amountText\.value[\s\S]*await subscribeEarnProduct\(selected\.value\.id, requestAmount\)/)
  assert.match(earnSource, /await redeemEarnSubscription\(subscription\.id\)/)
  assert.match(earnSource, /decimalWithinRange\(amountText\.value,[\s\S]*available: availableText\.value/)
})
...
test('预测页保留市场本地化、钱包、报价和订单确认链路', () => {
  ...
  assert.match(predictionApiSource, /refundAmount: predictionDecimal\(order\.refund_amount, 'refund_amount'\)/)
