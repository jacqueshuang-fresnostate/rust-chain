...
test('杠杆连续滑杆、设置、BBO、资产与主操作遵守 Pencil 几何和完整交互状态', () => {
  ...
  assert.match(cssRule('.contract-percentage'), /top: 240px;/)
  ...
  assert.match(cssRule('.contract-mode-row button,\n.contract-order-type'), /height: 38px;/)
  assert.match(tradeCss, /\n\.contract-order-type \{[^}]*height: 40px;/s)
  assert.match(cssRule('.contract-price-row > button'), /height: 54px;/)
  ...
  assert.match(cssRule('.contract-submit--long,\n.contract-trade .contract-submit--long.submit-order'), /top: 353px;/)
  assert.match(cssRule('.contract-submit--short'), /top: 435px;/)
