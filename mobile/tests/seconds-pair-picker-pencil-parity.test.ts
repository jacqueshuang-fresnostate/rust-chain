...
test('Seconds 交易对选择器复用共享模态生命周期并保持产品选择业务不变', () => {
  ...
  assert.match(secondsSource, /function handlePairPickerKeydown\(event: KeyboardEvent\): void \{/)
  assert.match(secondsSource, /moveRovingOptionId\([\s\S]*?filteredPairProductIds\.value/)
  assert.match(secondsSource, /isRovingListboxSelectionKey\(event\.key, event\.code\)/)
  assert.match(secondsSource, /trapPairPickerFocus\(event, closePairPicker\)/)
...
test('Seconds 交易对选择层 1:1 对齐 Pencil 07c 结构与真实数据', () => {
  ...
  assert.match(secondsSource, /:tabindex="activePairProductId === product\.id \? 0 : -1"/)
  assert.match(secondsSource, /:data-seconds-pair-option-id="product\.id"/)
  assert.match(secondsSource, /@focus="activePairProductId = product\.id"/)
  ...
  assert.match(secondsSource, /priceFor\(product\.symbol\)/)
