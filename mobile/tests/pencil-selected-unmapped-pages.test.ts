...
test('视觉重构没有移除真实接口、状态与危险操作复核', () => {
  ...
  assert.match(views.swap, /requestConvertQuote\(selectedPair\.value, amountNumber\.value\)/)
  assert.match(views.earn, /subscribeEarnProduct\(selected\.value\.id, amountNumber\.value\)/)
