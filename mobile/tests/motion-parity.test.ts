...
test('SignalField 限制画布像素并完整处理低动态、隐藏、尺寸与卸载', () => {
  ...
  assert.match(signalFieldSource, /const SIGNAL_FRAME_INTERVAL_MS = 1000 \/ 30/)
  assert.match(signalFieldSource, /timestamp - lastDrawTime >= SIGNAL_FRAME_INTERVAL_MS/)
  assert.match(signalFieldSource, /dataset\.performanceTier === 'constrained'/)
  assert.match(signalFieldSource, /staticMode\.value = reduced \|\| constrained[\s\S]*?const context = canvas\.getContext\('2d'\)/)
  assert.match(signalFieldSource, /if \(!context\) \{\s*staticMode\.value = true/)
  assert.match(signalFieldSource, /!document\.hidden && intersecting && !reduced && !constrained/)
  assert.match(signalFieldSource, /new IntersectionObserver/)
  assert.match(signalFieldSource, /intersectionObserver\?\.observe\(canvas\)/)
  assert.match(signalFieldSource, /intersectionObserver\?\.disconnect\(\)/)
  ...
  assert.match(signalFieldSource, /:class="\{ 'is-static': staticMode \}"/)
