...
test('BBO 做多回填卖一、做空回填买一，持仓与挂单按可空入场价分流', () => {
  ...
  assert.match(tradeSource, /visibleMarginPositions[\s\S]*?isFilledMarginPosition\(position\)/)
  assert.match(tradeSource, /visiblePendingMarginOrders[\s\S]*?isPendingMarginPosition\(position\)/)
