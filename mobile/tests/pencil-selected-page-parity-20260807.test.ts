...
test('资金划转按 Pencil 主弹窗与资产选择弹窗渲染，并保留真实钱包和可访问性合同', () => {
  ...
  assert.match(sources.assets, /const transferAvailableText = computed\(\(\) => decimalTextFromBoundary\(transferAccount\.value\?\.availableText,/)
  ...
  assert.match(sources.assets, /formatAmount\(account\.availableText\)/)
...
test('prediction and earn confirmations render only API-derived quote and product values', () => {
  ...
  assert.match(sources.prediction, /formatRatePercent\(selected\.yesPrice\)/)
  assert.match(sources.prediction, /formatRatePercent\(selected\.noPrice\)/)
  ...
  assert.match(sources.prediction, /selectedAccount\.value[\s\S]*?decimalWithinRange\(amountText\.value,[\s\S]*?available: selectedAccount\.value\.availableText,/)
  assert.match(sources.prediction, /return account \? formatAmount\(account\.availableText\) : '--'/)
  ...
  assert.match(sources.earn, /decimalMultiply\(amountText\.value, selected\.value\.aprRate\)[\s\S]*normalizeDecimalText\('365'\)/)
