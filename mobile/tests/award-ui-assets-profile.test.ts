...
test('资产页映射八个 Pencil 画板并只使用真实资产生成持仓', () => {
  ...
  assert.match(assetsSource, /\.filter\(\(row\) => decimalSign\(row\.amount\) > 0\)/)
  assert.match(assetsSource, /return decimalCompare\(right\.estimatedValue, left\.estimatedValue\)/)
  assert.match(assetsSource, /estimatedValue: estimateAssetValue\(row\.symbol, amount\)/)
  assert.match(assetsSource, /lastPrice \? decimalMultiply\(amount, lastPrice\) : null/)
