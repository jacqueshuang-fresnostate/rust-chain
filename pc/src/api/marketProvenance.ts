...
export function marketTradeIdentity(trade: MarketProvenance & { id?: string | number; time: number }): string {
  return JSON.stringify([trade.source ?? '', trade.provider ?? '', trade.id ?? trade.time])
}
