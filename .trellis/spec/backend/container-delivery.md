...
- Pull requests build without registry write permission. Pushes to `main`, `v*` tags, and manual
  dispatches publish multi-architecture `linux/amd64` and `linux/arm64` images through `GITHUB_TOKEN`.
...
| A pull request runs the workflow | Build both platforms, never log in or push |
| `main`, `v*`, or manual dispatch runs the workflow | Log in to GHCR and push generated tags |
...
- Bad: start the API and migration in parallel, use `depends_on` without health/completion
  conditions, commit `docker-compose.env`, or run the final container as root.

### 6. Tests Required

- Run `cargo fmt -- --check` and `cargo check --all-targets`.
- Parse the workflow and assert trigger, per-job permissions, platforms, tags, cache, and push policy.
