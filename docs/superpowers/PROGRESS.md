...
## 2026-08-28 15:21 - 手机端杠杆平仓弹窗与拖动确认对齐 Pencil

- 完成内容：以 Pencil 当前选中的浅色 `ajSJF` 与深色 `DGiNR` 平仓画板为基线，将杠杆持仓普通“平仓”改为 Teleport 底部弹窗；按 500px 面板、24px 顶部圆角、20px 内容边距、58px 价格/数量字段、69px 仓位摘要和 62px 粉色确认轨道复刻浅深主题。弹窗只展示当前真实交易对、方向、仓位模式、杠杆、风险快照标记价、可平数量与预计盈亏，缺失值显示 `--`。当前后端单仓接口仍为 100% 市价全平，因此比例轨道明确固定为 100%，未伪造部分平仓。新增从圆形手柄发起的 Pointer Capture 拖动确认，轨道点按不响应，低于 90% 松手回弹且不请求，越过阈值后通过防重守卫仅调用一次现有单仓平仓接口；同时覆盖键盘滑块语义、ARIA、焦点闭环、精确焦点恢复、滚动锁、请求中禁关、失败回弹重试、底部安全区和低动态模式。卡片“市价全平”二次确认与顶部批量平仓保持独立。
- 修改文件：`mobile/src/components/MarginCloseSheet.vue`、`mobile/src/core/slideToConfirm.ts`、`mobile/src/views/TradeView.vue`、`mobile/src/i18n/messages/{zh-CN,en}.ts`、`mobile/tests/{margin-close-sheet,contract-pencil-selected-parity}.test.ts`、`.trellis/spec/mobile/{backend-integration,pwa-and-shell}.md`、`.trellis/tasks/08-28-mobile-margin-close-slide-parity/**`、`docs/superpowers/PROGRESS.md`。
- 验证结果：杠杆平仓聚焦回归 37/37、Mobile 全量测试 493/493、`npm --prefix mobile run type-check`、`npm --prefix mobile run build:pwa`、`npm --prefix mobile run build:tauri` 与 `git diff --check` 均通过；项目未配置独立 Mobile lint script。Ego Browser 使用本地内存模拟仓位验证 448×900 浅色面板宽 448px、高 500px，确认轨道 408×62px、左右边距 20px；既有验证覆盖 390×844 与 320×720 深色安全区和零横向溢出。实际点击轨道末端后进度仍为 0、弹窗保持打开且平仓请求数为 0；从手柄拖至阈值以下后回弹到初始位置，请求数仍为 0，正常打开流程中 body 滚动保持锁定。未越过阈值，未执行真实资金操作。
- 后续事项：无；本轮改动尚未提交或推送，工作区此前未提交的杠杆字段布局、根导航弹窗、Pencil 文件及其测试改动均保持原样。

## 2026-08-28 18:53 - 杠杆比例拖动与真实部分平仓结算

- 完成内容：按用户澄清把 `margin-close-sheet__ratio` 从固定 100% 展示改为独立的 1%–100% 原生连续拖动条，鼠标、真触控、触控笔和键盘均可改变本次平仓比例，并即时同比例更新可平数量与预计盈亏；底部粉色滑杆继续只承担防误触确认。确认时冻结仓位、比例和幂等键，失败原键重试，成功后通过钱包、仓位和风险快照重新对账。后端单仓平仓接口新增可选 `percentage`/`idempotency_key`，历史空请求仍按 100% 全平；新增不可变部分平仓执行表，按事务内剩余保证金、名义价值、借款和利息同比例切片，十八位向下取整并以减法保留精确余量，逐仓/全仓资金、流水、执行记录、剩余仓位和账户版本同事务提交。补齐同键重放/异参冲突、私有刷新事件、累计已实现盈亏、后续强平不覆盖历史切片，以及今日收益按执行日计入部分平仓并排除显式终态重复统计的完整口径。
...
## 2026-09-05 03:45 - 完成手机新币 Pencil 八画板复刻与公开读模型

- 完成内容：按选定八张 390×920 明暗 Pencil 画板重建打新专区、交易先机、打新详情和统一打新记录；复制并校验选定横幅为独立生产资源，拆分项目/机会/记录卡片与严格 DTO/DecimalText 展示助手，接入后台资产名称/Logo、权威计价资产、供给字段、共享行情租约、真实认购/购买/手续费/释放动作、四源时间倒序记录、双语文案、窄屏与无障碍模态；派发卡明确展示真实入账或锁仓编号，上市后购买比例、复核和请求统一使用后端强校验的项目发行价，不把行情展示值误作执行价。后端公开列表/详情使用同一联表读模型返回项目及计价资产名称/Logo。针对访客回归新增显式公开请求标记：公开项目和行情请求不携带陈旧 Bearer、不触发刷新或登录过期跳转，专区只在打开交易先机时初始化行情，钱包/记录等私有请求在认证前短路；登录跳转仅保留给实际可用的申购、购买及记录动作。路由样式按组件拆分，并把新增八画板 CSS 的总量门禁由 640 KiB 定向重基线到 656 KiB，实际 gzip 门禁仍保持 128 KiB。
- 修改文件：`mobile/src/{api/{client,market,newCoin,requestAuth}.ts,core/{newCoinModel,newCoinPresentation}.ts,components/{PageHeader,new-coin/*}.vue,views/{NewCoins,NewCoinDetail,NewCoinRecords}View.vue,i18n/messages/{zh-CN,en}.ts,styles/pencil-selected-pages.css,assets/new-coin-launch-banner.jpg}`、`mobile/scripts/{check-source-size,check-bundle-budget}.mjs`、`mobile/tests/{new-coin-model,new-coin-presentation,new-coin-pencil-contract,request-layer,android-ui-secondary-prototype,new-coin-purchase,pencil-selected-page-parity-20260807,pencil-selected-unmapped-pages,secondary-product-order-views}.test.ts`、`src/modules/new_coin/{repository,infrastructure,presentation}.rs`、`tests/new_coin_routes.rs`、`.trellis/spec/{backend/new-coin-mobile-contract,mobile/backend-integration,mobile/pwa-and-shell}.md`、`docs/superpowers/PROGRESS.md`。
- 验证结果：`npm --prefix mobile run release:gate` 全链路通过，生产/测试类型检查、Mobile 全量 670/670、PWA/Tauri production build 与产物检查、Bundle（CSS 649.9/656.0 KiB raw、121.2/128.0 KiB gzip）、源码尺寸和关键测试质量门禁均绿色；`cargo fmt --check && cargo check && cargo test --test new_coin_routes && cargo clippy --all-targets --all-features -- -D warnings` 通过，路由测试报告 11/11，`cargo test new_coin` 另有 15/15 个库内新币相关单元测试通过。因本机未配置 `DATABASE_URL`，路由中的 9 个 MySQL 实库分支（含新增公开列表/详情字段完全一致断言）按既有合同明确跳过，2 个无库错误/鉴权用例实际执行；Ego 在 390×920 明暗主题完成八状态视觉、焦点/Escape/遮罩、访客路由与请求回归，并确认 320/448px 零横向溢出，最终访客停留 `#/products/new-coins`、卡片 358×300/358×140、详情 210/112/104/328px、无未翻译键或控制台错误；横幅源/生产副本 SHA-256 均为 `becbd0fbbc86c66082a163f2358fe1878f790b4591a34fc4fb84ab857e1cdc36`，生产源码无 `mobile/pencil` 依赖，`git diff --check` 通过。
- 后续事项：有可用 MySQL 时复跑 `cargo test --test new_coin_routes public_new_coin_routes_return_authoritative_asset_metadata -- --nocapture` 取得真实联表运行证据；本轮未编辑 `.pen`，保留全部无关 Pencil 脏文件，未提交或推送。

## 2026-09-05 05:13 - 独立复核并收口手机新币 Pencil 八画板

- 完成内容：逐项复核 PRD、注入规范、两份研究文档和八张 390×920 导出图，并以 390×892 Web body 运行时锁定专区首卡 `x=16, y=344, 358×300`、标题后 12px 间距、机会卡 358×140、详情 210/112/104/328px 分区及记录卡 358×168/14px 间距；修复交易先机误受上市后购买开关过滤、三种解禁枚举裸露、缺失名称伪装为符号、HIPPO 长供给截断、记录状态色分叉和 fixed 弹层受路由 transform 限制。公开请求只在列表/详情与行情读取时剥离陈旧凭证并禁止会话刷新，记录、钱包及写动作继续受保护；项目/计价资产名称与 Logo 由公开后端 LEFT JOIN DTO 权威提供，机会只持有一个共享实时行情租约。按范围纠正复核 `src/modules/new_coin/infrastructure.rs`，仅保留公开 list/detail 资产联表，订阅/购买订单写入、供给会计、结算与既有锁定语义均保持原样。
- 修改文件：`mobile/src/{api/{client,market,newCoin,requestAuth}.ts,core/{newCoinModel,newCoinPresentation}.ts,components/{PageHeader,new-coin/*}.vue,views/{NewCoins,NewCoinDetail,NewCoinRecords}View.vue,i18n/messages/{zh-CN,en}.ts,styles/pencil-selected-pages.css}`、`mobile/scripts/{check-source-size,check-bundle-budget}.mjs`、`mobile/tests/{new-coin-model,new-coin-presentation,new-coin-pencil-contract,request-layer,android-ui-secondary-prototype,new-coin-purchase,pencil-selected-page-parity-20260807,pencil-selected-unmapped-pages,secondary-product-order-views}.test.ts`、`src/modules/new_coin/{repository,infrastructure,presentation}.rs`、`tests/new_coin_routes.rs`、`.trellis/spec/{backend/new-coin-mobile-contract,mobile/backend-integration,mobile/pwa-and-shell}.md`、`.trellis/tasks/09-05-mobile-new-coin-pencil-parity/{prd.md,review.md}`、`docs/superpowers/PROGRESS.md`。
- 验证结果：Mobile 聚焦 5 文件 30/30、生产/测试 TypeScript 类型检查及治理门禁通过；最终 Mobile 源码状态的 `release:gate` 通过，全量 672/672、PWA/Tauri production build 与产物检查通过，CSS Bundle 650.6/656 KiB raw、121.4/128 KiB gzip。Rust `cargo fmt --all -- --check`、`cargo check --all-targets --all-features`、`cargo test new_coin`、Clippy `-D warnings` 通过；本机一次性 MySQL 库实际执行公开列表/详情权威资产元数据聚焦测试 1/1，通过后已删除。明暗主题及 320/390/448px 运行时无水平溢出、控制台 warning/error 为空；`git diff --check` 与 Trellis task validate 通过。
- 后续事项：既有真实 MySQL 全套路由测试中，`concurrent_new_coin_subscriptions_never_allocate_beyond_remaining_supply` 单项曾返回 `[200, 500]`，既有断言期望 `[200, 400]`，同次其余 10 项通过；该订单写/供给并发语义早于且超出本 PRD，应另立后端任务分析，本轮未为此改变写路径。未编辑 `.pen`，未还原或覆盖既有 `mobile/pencil` 脏文件，未提交或推送。

## 2026-09-05 15:31 - 补齐打新专区资产 Logo 与发行价币种显示

- 完成内容：复核线上公开新币响应并确认当前部署镜像尚未返回 `name/logo_url/quote_asset_symbol/quote_asset_logo_url`；保持公开新币列表/详情通过 `asset_id`、`quote_asset_id` 联表 `assets` 的权威元数据合同，项目卡继续把项目资产 `logo_url` 直接交给 `AssetMark`。将发行价金额和计价资产符号拆为两个布局槽，长金额只在金额槽内省略，计价符号使用非收缩槽稳定显示，不按符号拼接图片 URL，也不硬编码 USDT。补充线上缺口研究、PRD、复核记录与 Mobile/Backend 可执行规范。
