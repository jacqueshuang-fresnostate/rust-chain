...
function mockEmptyResource() {
  listAdminResourceMock.mockResolvedValue({ rows: [], raw: {} });
...
describe('resourceConfigs create actions', () => {
  ...
  it('opens a spot trading pair creation modal from the trading pair config page', async () => {
    ...
    await user.click(screen.getByRole('button', { name: '提交添加交易对' }));
    await user.type(screen.getByLabelText('操作原因'), 'add spot pair');
    await user.click(screen.getByRole('button', { name: '确认' }));

    await waitFor(() => {
      expect(apiRequestMock).toHaveBeenCalledWith('/admin/api/v1/market-pairs', expect.objectContaining({ method: 'POST' }));
    });
  });
  ...
  it('opens a margin trading pair creation modal from the margin product page', async () => {
    ...
    await user.click(screen.getByRole('button', { name: '提交添加杠杆交易对' }));
  ...
  it('opens a seconds contract pair creation modal from the seconds product page', async () => {
    ...
    await user.click(screen.getByRole('button', { name: '提交添加秒合约交易对' }));
    await user.type(screen.getByLabelText('操作原因'), 'add seconds pair');
    await user.click(screen.getByRole('button', { name: '确认' }));

    await waitFor(() => {
      expect(apiRequestMock).toHaveBeenCalledWith('/admin/api/v1/seconds-contracts/products', expect.objectContaining({ method: 'POST' }));
    });
  });
});
