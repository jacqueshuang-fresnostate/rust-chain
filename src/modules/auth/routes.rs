use crate::{
    ...
    modules::auth::{
        AdminCredentials, AdminRegistration, AgentCredentials, AgentRegistration, AuthService,
        IssuedTokens, MySqlAuthRepository, TokenScope, UserCredentials,
    },
    state::AppState,
};
...
async fn user_refresh(
    ...
    refresh(&state, request.refresh_token).await
}
...
async fn refresh(
    ...
    let tokens = auth_service(state)?.refresh(refresh_token).await?;
...
mod tests {
    ...
    async fn agent_auth_routes_return_clear_error_without_mysql() {
        ...
        assert_auth_route_requires_mysql(
            app.clone(),
            "/auth/register",
            r#"{"username":"agent","password":"password-1","agent_id":1}"#,
        )
        .await;
        assert_auth_route_requires_mysql(
            app.clone(),
