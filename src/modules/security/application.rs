...
//!
//! 应用层：编排用例、事务边界和跨仓储协作。
//! 本文件编排安全校验用例：登录二次验证挑战的可用性判定、用户与管理员的 TOTP 校验，
//! 以及资金动作发起前按策略施加的资金密码与动态码组合校验。
//! 职责分工是把领域层的纯算法与基础设施层的存取拼装起来：
//! 从数据库取出加密密钥，用凭证加密密钥解密，交给领域函数比对，通过后回写最近验证时间。
//! 密钥解密只在本层内存中发生，明文密钥不向上返回也不向下持久化。
//! 挑战判定与消费刻意分离：本层的判定函数只读快照且不加锁，
//! 因此判定通过与后续写入之间存在竞态窗口，真正的防重放需由调用方消费挑战来实现。
//! 各函数注释会分别标明其是否已完成消费，调用方不得仅凭判定成功就认为挑战已被独占。
