...
/// 保存用户在指定杠杆产品上的默认杠杆倍数：兼容旧单倍数载荷，也接受完整的做多/做空双值载荷。
/// 两个方向都必须精确命中产品配置档位；用例锁定启用产品后原子写入，只影响后续开仓默认值，
/// 不改动已开仓位的杠杆、保证金、名义价值或强平状态。
...
/// 读取用户在指定杠杆产品上已保存的保证金模式、兼容单倍数及做多/做空双方向倍数，供交易页初始化下单面板。
