...
/// 按最新服务端行情实时计算单个仓位的浮盈、权益、维持保证金和是否触及强平线。
/// 这是本文件唯一一个需要 Redis 的只读入口，行情缺失、超过六十秒或价格非正都会直接失败。
...
/// 杠杆市价开仓入口，是本模块风险最高的资金写路径，同时依赖连接池、Redis 行情和事件广播中心。
/// 请求必须携带幂等键；用例先按键做只读重放检查，再在事务内锁产品、写仓位占键、扣抵押并记录返佣。
/// 入场价一律取服务端缓存行情，请求体不允许传 price 或 trigger_price，传了直接判为参数非法。
/// 只有事务提交且确实新建了仓位，才会在提交后向用户私有频道推送开仓事件。
