//! 后台用户账号与人工充值的纯业务规则层。
//!
//! 包含创建用户与人工加币两条写入路径的请求校验、用户状态归一、口令散列包装，
//! 以及用户档案与充值结果两类审计快照。所有联系方式唯一性、资产是否可用、余额是否足够
//! 都不在这里判断，由 application 层在事务内完成；明文口令只经过散列函数，不进入任何审计结构。
...
/// 邮箱与手机号是二选一而非都必填，只要有一个非空即可；邮箱只做长度不超 255 且含 `@` 的弱校验，
/// 手机号只限长 32 字节，均不校验真实可达性。口令必填但强度由散列环节的认证域规则把关。
/// 初始状态为可选，缺省由应用层落为启用；实名等级缺省为 0 且不得为负；审计原因在此强制必填。
