//! 闪兑交易对配置与闪兑订单查询的应用用例层。
//!
//! 三个只读用例都不加锁也不计算报价，返回的是数据库中保存的配置与已成交流水。
//! 三个写用例共享同一套编排：开事务、锁定交易对、校验合并后的完整配置、写入、回读、写审计、提交，
//! 且都强制要求审计原因。删除额外要求交易对已停用且不存在报价、订单与新币兑换规则引用。
...
/// 状态不做枚举校验，未知取值得到的是空结果而非错误；邮箱按原值下推参与关联匹配。
/// 每条订单保留成交当时的汇率与手续费，因此后续修改交易对配置不会改变历史订单的呈现口径。
