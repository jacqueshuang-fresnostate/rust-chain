//! 站内新闻内容管理的应用用例层。
//!
//! 写入路径刻意把「改内容」与「改状态」拆成两个用例：前者不动发布状态与首发时间，
//! 后者不动标题正文，从而避免一次操作同时改变展示内容与对外可见性而难以回溯。
//! 三个写用例都在事务内完成锁定、写入、回读与审计，且都要求审计原因；
//! 全程不发送站内信、不广播事件、也不清理对象存储中被替换掉的旧图片。

use super::*;

/// 按状态、分类、国家、locale 和关键字筛选后台新闻，并返回分页摘要和匹配总数。
/// 枚举与地域筛选沿用写入校验，关键字仅去空白；查询不锁新闻且不返回富文本写事务快照。
/// 复用写入侧校验意味着非法状态、分类或地域会直接报校验错误，而不是当作查无记录返回空列表。
/// 返回的是不含完整正文的摘要视图，富文本需按编号另取详情，以免列表响应因正文过大而膨胀。
...
/// 由于审计快照刻意不含富文本正文，仅改正文的操作在前后值上只表现为更新人和更新时间变化，
/// 具体改了什么需要依赖本次提交的操作原因来说明。
