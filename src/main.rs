//! 交易所 API 服务的可执行入口：初始化日志、建立全部外部连接、按开关拉起后台协程，最后启动 HTTP 监听。
//! 外部依赖在这里全部为必需项，任何一项连接失败都会让进程直接退出，避免服务带着残缺依赖对外提供交易能力。
//! 后台协程一律用 `tokio::spawn` 以「发射后不管」的方式启动，循环内部异常只写错误日志，不会反向终止主进程。
//! 因此某个 worker 静默退出时 HTTP 接口仍然可用，排查结算、解禁、行情类问题必须回看对应协程的停止日志。
...
/// 完成服务的整体启动装配，按固定顺序推进：日志订阅、配置解析、外部连接、共享状态、后台协程、HTTP 监听。
/// 日志过滤器优先取自环境变量，缺失时回落到 info 级别；配置与 MySQL、Mongo、Redis、认证存储、RabbitMQ 的连接
